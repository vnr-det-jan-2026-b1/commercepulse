...
Think like a bootstrapped founder, not a consultant. Be direct, specific, and aggressive about growing sales."""
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            llm = ChatGroq(api_key=key, model="llama-3.3-70b-versatile", temperature=0.1).with_fallbacks([ChatGroq(api_key=key, model="llama3-8b-8192", temperature=0.1)]).with_config(tags=["synthesizer"])
